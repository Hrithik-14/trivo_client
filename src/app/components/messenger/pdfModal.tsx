    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[9999]">
      <div className="bg-white w-[90%] h-[90%] rounded-xl shadow-xl flex flex-col">
        <div className="flex justify-between items-center p-3 border-b">
          <button onClick={handleDownload} className="cursor-pointer text-black font-bold">
            <Download size={18} />
          </button>
          <h2 className="text-sm font-semibold text-black truncate max-w-[70%]">
            {fileName}
          </h2>
          <button onClick={() => setOpen(false)} className="text-red-500 font-bold text-lg rotate-45">
            <Plus />
          </button>
        </div>
