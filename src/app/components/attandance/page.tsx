    ...
    <div className="bg-white border border-gray-200 rounded-lg p-4">
      ...
      <div className="grid grid-cols-7 gap-1">
        {days.map((day, index) => {
          ...
          const customClassName = tileClassName
            ? tileClassName({ date: day, view: "month" })
            : "";
...
const AttendancePage: React.FC = () => {
  ...
  const [regularisationHistory, setRegularisationHistory] = useState<
    LeaveDes[]
  >([]);
    ...
    try {
      ...
      await api.post(
        "/notification",
        {
          ...
          type: "leaveRequest",
          action: "sent",
          entityId: response.data.leave._id,
          description: `Leave Request: ${leaveForm.leaveType}`,
        },
        {
          headers: {
            "Content-Type": "application/json",
          ...
          {dayStatus.status !== "absent" && (
            <div className="flex items-center gap-3">
              ...
              <span className="text-gray-700">
                {formatTime(dayStatus.signInTime)}
              </span>
            </div>
          )}
          ...
          {des && (
            <>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-gray-700">
                  {des.description || "No reason provided"}
                </span>
              </div>
            </>
          )}
      ...
      <div className="flex justify-between">
        <div className="flex flex-wrap gap-4 justify-center lg:justify-start">
          <p className="w-40 flex flex-col items-center border py-2 border-[#ddd] bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
            <div
              className={`text-4xl font-bold mb-2 ${
                leaveCount?.sickCount == 8 ? "text-red-500" : "text-green-500"
              }`}
            >
              {leaveCount?.sickCount}/<span className="text-[#696969]">8</span>
            </div>
            <span className="text-[#696969] font-semibold text-sm text-center">
              Total Sick <br /> Leave
            </span>
          </p>
          ...
          <p className="w-40 flex flex-col items-center border py-2 border-[#ddd] bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
            <div
              className={`text-4xl font-bold mb-2 ${
                leaveCount?.PaternityCount == 12
                  ? "text-red-500"
                  : "text-green-500"
              }`}
            >
              {leaveCount?.PaternityCount}/
              <span className="text-[#696969]">12</span>
            </div>
            <span className="text-[#696969] font-semibold text-sm text-center">
              Total Paternity <br /> Leave
            </span>
          </p>
          ...
          <p className="w-40 flex flex-col items-center border py-2 border-[#ddd] bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
            <div
              className={`text-4xl font-bold mb-2 ${
                leaveCount?.MaternityCount == 90
                  ? "text-red-500"
                  : "text-green-500"
              }`}
            >
              {leaveCount?.MaternityCount}/
              <span className="text-[#696969]">90</span>
            </div>
            <span className="text-[#696969] font-semibold text-sm text-center">
              Total Maternity <br /> Leave
            </span>
          </p>
          ...
          <p className="w-40 flex flex-col items-center border py-2 border-[#ddd] bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
            <div
              className={`text-4xl font-bold mb-2 ${
                leaveCount?.CasualCount == 6 ? "text-red-500" : "text-green-500"
              }`}
            >
              {leaveCount?.CasualCount}/
              <span className="text-[#696969]">6</span>
            </div>
            <span className="text-[#696969] font-semibold text-sm text-center">
              Total Casual <br /> Leave
            </span>
          </p>
          ...
          <p className="w-40 flex flex-col items-center border py-2 border-[#ddd] bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
            <div
              className={`text-4xl font-bold mb-2 ${
                leaveCount?.PrivilegeCount == 15
                  ? "text-red-500"
                  : "text-green-500"
              }`}
            >
              {leaveCount?.PrivilegeCount}/
              <span className="text-[#696969]">15</span>
            </div>
            <span className="text-[#696969] font-semibold text-sm text-center">
              Total Privilege <br /> Leave
            </span>
          </p>
          ...
          <p className="w-40 flex flex-col items-center border py-2 border-[#ddd] bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200">
            <div
              className={`text-4xl font-bold mb-2 ${
                leaveCount?.CompOffCount == leaveCount?.CompOffHave
                  ? "text-red-500"
                  : "text-green-500"
              }`}
            >
              {leaveCount?.CompOffCount}/
              <span className="text-[#696969]">{leaveCount?.CompOffHave}</span>
            </div>
            <span className="text-[#696969] font-semibold text-sm text-center">
              Total CompOff <br /> Leave
            </span>
          </p>
        </div>
        <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={
                      leaveForm.leaveDate
                        ? leaveForm.leaveDate.toISOString().split("T")[0]
                        : ""
                    }
                    onChange={(e) =>
                      setLeaveForm({
        ...
        {leaveHistory.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            {leaveHistory.map((leave) => (
              <div
                key={leave._id}
                className="shadow-sm border border-gray-300 rounded-lg p-4 flex flex-col gap-2"
              >
                <div className="flex gap-3">
                  <h2
                    className={`w-fit px-3 text-xs py-1 rounded-full font-semibold ${
                      leave.leaveType === "Sick"
                        ...
                        : leave.leaveType === "Privilege"
                        ? "text-purple-700 bg-purple-100 border border-purple-300"
                        : leave.leaveType === "Regularization"
