    ...
    const fetchStatuses = async () => {
      try {
        const response = await api.get(
          `/report/getReportsByEmployee/${user?.id}`,
          {
  ...
  return (
    <div className="min-h-screen  py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div
          className={`space-y-4 ${
            showStyle === true ? "" : "grid grid-cols-2 gap-5"
          }`}
        >
          {filteredReports.length > 0 ? (
            filteredReports.map((report, index) => (
              <div
                key={report.id || index}
                className="p-6 bg-white hover:bg-gray-50 rounded border border-[#ddd] h-fit"
              >
            ...
            <p className="w-full text-center text-lg pt-5 font-semibold">
              No reports found
            </p>
          )}
        </div>
